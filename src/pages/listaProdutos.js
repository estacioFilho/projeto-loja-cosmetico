...
async function produtosFromAPI(categoria, titulo = categoria) {
    ...
    try {
        utilities.adicionarTextoTitulo(titulo, paginaDestaque, tituloPagina);
        const produtos = await utilities.listaProdutos('product');
        
        const produtosFiltrados = produtos.filter(item => item.category === categoria && item.stock);
        ...
        document.querySelectorAll('.button-produto').forEach((botao, index) => {
            botao.addEventListener('click', () => adicionarAoCarrinho(produtosFiltrados[index], categoria));
        });
